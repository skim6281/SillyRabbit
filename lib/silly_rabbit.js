...
let stage, width, height, loader, messageField, scoreField, score;
let hitCount;
...
function watchForRestart() {
  stage.removeChild(carrot, carrotRect, rabbitRect, scoreField);
  score = 0;
  stage.addChild(messageField);
  stage.update();
...
function start() {
  ...
  stage.addChild(bg, bush, grass,carrotRect,carrot, ground, cereal, rabbitRect, rabbit, scoreField);

  if (createjs.Ticker.hasEventListener("tick")) {
    console.log("in ticker if statement");
...
function handleTick(e) {
  ...
  scoreField.text = (Number(scoreField.text) + 1).toString();
...
function onFaint(e) {
  ...
  if (hitCount === 1) {
    ...
    messageField.text = `Game over! Score: ${scoreText}\n Click to play again`;
  }
  stage.addChild(messageField);
  watchForRestart();
}
