...
function start() {
  ...
  rabbitHitBox = new createjs.Shape();
  rabbitHitBox.graphics.beginFill("#000").drawRect(rabbit.x - 17, rabbit.y - 20, 60, 40);
  rabbitHitBox.alpha = 0;

  stage.addChild(bg, bush, grass)
  for(let i = 0; i < 3; i ++) {
    stage.addChild(carrots[i], carrotHitBoxs[i]);
  }
  stage.addChild(ground, cereal, rabbitHitBox, rabbit, scoreField);
...
function handleJump(e) {
  if (e.keyCode === 32 && onGround() && !gotHit) {
    ...
    createjs.Tween.get(rabbit)
      ...
      .to({y: 240}, 100)
      .to({y: 310}, 250)
    createjs.Tween.get(rabbitHitBox)
      .to({y: -100}, 150)
      .to({y: -120}, 150)
      .to({y: -140}, 170)
      .to({y: -120}, 160)
